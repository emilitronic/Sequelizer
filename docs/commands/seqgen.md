...
Sequelizer's `seqgen` subcommand creates physical signals from textual DNA sequences. 
 ...
 It can read sequences from FASTA/FASTQ files or generate random sequences (i.e., ACGT strings), and simulate the nanopore signals that would be produced when those sequences are processed by a nanopore sensor.  You might have seen a basic example in [Examples](../guides/examples.md).
